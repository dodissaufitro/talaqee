    ...
    return (
        ...
        <div className="hidden md:block min-h-screen bg-gray-50 font-sans selection:bg-[#7e57c2] selection:text-white">
            ...
            <div className="block md:hidden bg-white min-h-screen pb-24 font-sans selection:bg-blue-600 selection:text-white">
                ...
                <div className="px-5 mb-6">
                    <div className="flex overflow-x-auto gap-4 snap-x snap-mandatory hide-scrollbar pb-1">
                        {banners.length > 0 ? banners.map((banner) => (
                            <Link href={banner.link_url || '#'} key={banner.id} className="flex-none w-full snap-start block">
                                <div className={`w-full aspect-[2.2/1] rounded-2xl overflow-hidden shadow-sm relative group flex items-center ${banner.background_color || 'bg-blue-50'} transition-transform duration-300 hover:shadow-md`}>
                                    {/* Text Content (Left) */}
                                    <div className="w-[55%] h-full flex flex-col justify-center px-6 py-4 z-10 relative">
                                        {banner.title && (
                                            <h2 className="text-xl md:text-2xl lg:text-3xl font-extrabold text-[#1a202c] leading-tight mb-2" dangerouslySetInnerHTML={{ __html: banner.title }} />
                                        )}
                                        {banner.subtitle && (
                                            <p className="text-gray-600 text-xs md:text-sm lg:text-base line-clamp-3 mb-4 max-w-sm">
                                                {banner.subtitle}
                                            </p>
                                        )}
                                        {banner.button_text && (
                                            <div className="mt-auto md:mt-2">
                                                <button className="bg-blue-600 hover:bg-blue-700 text-white text-xs md:text-sm font-semibold px-4 md:px-6 py-2 md:py-2.5 rounded-full flex items-center gap-2 transition-colors">
                                                    <Search size={16} /> {banner.button_text}
                                                </button>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="absolute right-0 bottom-0 h-full w-[50%] flex items-end justify-end pointer-events-none">
                                        <img 
                                            ...
                                            className="h-full w-auto object-cover object-left mask-image-to-l group-hover:scale-105 transition-transform duration-700" 
                ...
                {/* Features Info */}
                <div className="px-5 mb-8">
                    <div className="grid grid-cols-2 gap-x-4 gap-y-6 bg-[#F8FAFC] rounded-[20px] p-5 border border-gray-100">
                        
                        <div className="flex flex-col gap-2">
                            <div className="w-10 h-10 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center shrink-0 mb-1">
                                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4Z"></path><path d="M3 6h18"></path><path d="M16 10a4 4 0 0 1-8 0"></path></svg>
                            </div>
                            <div>
                                <h4 className="text-[12px] font-bold text-gray-900 mb-1">Koleksi Lengkap</h4>
                                <p className="text-[10px] text-gray-500 leading-tight">Ribuan buku dari berbagai genre terlengkap.</p>
                            </div>
                        </div>

                        <div className="flex flex-col gap-2">
                            <div className="w-10 h-10 bg-emerald-50 text-emerald-500 rounded-full flex items-center justify-center shrink-0 mb-1">
                                <Shield className="w-5 h-5" />
                            </div>
                            <div>
                                <h4 className="text-[12px] font-bold text-gray-900 mb-1">100% Original</h4>
                                <p className="text-[10px] text-gray-500 leading-tight">Buku original dan berkualitas langsung dari penerbit.</p>
                            </div>
                        </div>

                        <div className="flex flex-col gap-2">
                            <div className="w-10 h-10 bg-purple-50 text-purple-500 rounded-full flex items-center justify-center shrink-0 mb-1">
                                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="16" height="13" x="6" y="8" rx="2"></rect><path d="M10 8V6a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v2"></path><path d="M2 12h4"></path><path d="M2 16h4"></path></svg>
                            </div>
                            <div>
                                <h4 className="text-[12px] font-bold text-gray-900 mb-1">Pengiriman Cepat</h4>
                                <p className="text-[10px] text-gray-500 leading-tight">Pengiriman aman dan cepat ke seluruh Indonesia.</p>
                            </div>
                        </div>

                        <div className="flex flex-col gap-2">
                            <div className="w-10 h-10 bg-orange-50 text-orange-500 rounded-full flex items-center justify-center shrink-0 mb-1">
                                <Headphones className="w-5 h-5" />
                            </div>
                            <div>
                                <h4 className="text-[12px] font-bold text-gray-900 mb-1">Layanan 24/7</h4>
                                <p className="text-[10px] text-gray-500 leading-tight">Kami siap membantu kapan saja yang Anda butuhkan.</p>
                            </div>
                        </div>
                ...
                <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-[#F1F5F9] md:max-w-md md:mx-auto z-50">
                    <div className="flex justify-around items-center h-[70px] pb-2">
                        {[
                            ...
                            <Link href={item.route} key={item.id} className="flex flex-col items-center justify-center w-16 gap-1 relative mt-1">
                                {item.active ? (
                                    <>
